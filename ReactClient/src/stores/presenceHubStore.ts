...
export default class PresenceHubStore {
    ...
    createHubConnection = (user: IUser) => {
        ...
        this.hubConnection.start().catch(error => console.log('Erro ao estabelecer a conexão: ', error));
        ...
        this.hubConnection.on('NewMessageReceived', (member: IMember) => {
            runInAction(() => {
                //para exibir a caixa de bate-papo do nome de usuário, envie uma mensagem
    ...
    stopHubConnection = () => {
        this.hubConnection?.stop().catch(error => console.log('Erro ao interromper a conexão: ', error));
    }
