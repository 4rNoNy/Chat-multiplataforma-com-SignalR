...
class PresenceHubController extends GetxController {
  ...
  void createHubConnection(User? user) {
    if (_hubConnection == null) {
      ...
      if (_hubConnection!.state != HubConnectionState.Connected) {
        _hubConnection!
            .start()
            ?.catchError((e) => {print("PresenceService no início: $e")});
      }
      ...
      _hubConnection!
          .on("DisplayInformationCaller", _displayInformationCallerReceived);
  ...
  void _displayInformationCallerReceived(List<Object?>? parameters) {
    if (Global.myStream!.isInForeground) {
      ...
      Global.myStream!.navigateToScreen(memberCalling); // espera bottom_navbar
    }
  }
  ...
  void _getOnlineUsers(List<Object?>? parameters) {
    ...
    final listMember =
        memberServer.map<Member>((json) => Member.fromJson(json)).toList();
  ...
  void _newMessageReceived(List<Object?>? parameters) {
    ...
    int index = lastMessages.indexWhere((f) =>
        f.senderUsername == member.userName!); //message['senderUsername']
    if (index != -1) {
  ...
  void stopHubConnection() {
    _hubConnection!
        .stop()
        .catchError((e) => {print("Hub de presença na parada: $e")});
    _hubConnection = null;
  }
  ...
  Future<void> callToUser(String ortherUsername, String channelName) async {
    await _hubConnection!
        .invoke("CallToUsername", args: <Object>[ortherUsername, channelName]);
  }
  ...
  Future<String> fetchLastMessages() async {
    ...
    try {
      ...
      final uri =
          Uri.parse('$urlBase/api/LastMessageChats?pageNumber=1&pageSize=5');
      ...
      if (response.statusCode == 200) {
        ...
        lastMessages.value = list
            .map<LastMessageChat>((json) => LastMessageChat.fromJson(json))
            .toList();
        messageRes = '200';
